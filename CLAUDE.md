...
The web application (`apps/web`) can be deployed to Vercel for automatic CI/CD and hosting.

### Initial Setup (One-time)

**IMPORTANT**: To prevent duplicate projects and unstable URLs, link to a single Vercel project:
...
# - Link to existing project? → Yes (if exists) or No (for first time)
# - Project name: myapp-web (or your preferred name)
```

**This creates `.vercel/` directory that locks the project to a single Vercel deployment.**

### Vercel Dashboard Configuration

After linking, configure in Vercel Dashboard (Settings → General):

1. **Root Directory**: `apps/web` ← **Critical for monorepo**
2. **Framework Preset**: Next.js (auto-detected)
3. **Build Command**: Uses `apps/web/vercel.json` configuration
4. **Install Command**: Vercel uses default `npm install`
5. **Node.js Version**: 18.x (configured in `vercel.json`)

### Git Integration Setup

**Important**: Ensure only ONE Vercel project is connected to your Git repository.

1. Go to Vercel Dashboard → Settings → Git
2. Verify only one project is connected
3. If multiple projects exist, delete old/duplicate ones
...
- **Root `vercel.json`**: Minimal config (version only)
- **`apps/web/vercel.json`**: App-specific build configuration
- **`.vercelignore`**: Excludes Android artifacts, build outputs
- **`.vercel/`**: Project link (git-ignored, local only)

### Build Process

When Root Directory is set to `apps/web`, Vercel:

1. Runs install from monorepo root: `npm install --no-optional --force`
2. Builds shared package: `npm run build:shared --prefix ../..`
3. Builds web app: `npm run build --prefix ../..`
4. Deploys only `apps/web/.next` output
...
### URL Structure (After vercel link)

- **Production**: `https://myapp-web.vercel.app` (fixed URL)
- **Preview**: `https://myapp-web-git-branch-name-team.vercel.app`
- **Custom Domain**: Configure in Dashboard → Settings → Domains
...
**Problem**: New project created on each deployment, URL keeps changing
**Solution**:
- Delete duplicate projects in Vercel Dashboard
- Run `vercel link` locally to establish single project link
- Set **Root Directory** to `apps/web` in Dashboard settings
- Verify Git integration points to only ONE project

**Problem**: Build fails with "Cannot find module '@myapp/shared'"
**Solution**:
- Ensure `apps/web/vercel.json` includes build:shared step
- Verify `installCommand` runs from monorepo root
- Check `apps/web/next.config.js` has `transpilePackages: ["@myapp/shared"]`
